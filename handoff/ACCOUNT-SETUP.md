...
| 5 September 2026 | Transactional push, code 0 at last, and the token binding proved | Three sends of `low_balance` (`f7f833ef-...`). To `DPS-DTELCO-1788589415987`, the anonymous key the browser actually subscribed under: contact route, code 0, sent, a real notification. To `DPS-DTELCO-1` by contact key: code 11, because the token binds to the key that subscribed and `setContactKey` to the persona does not rebind it, which is `reference` section 4 stated as a live fact rather than a warning. To `DPS-DTELCO-1` again with the device token passed: token route, code 0, sent. So a persona picked after a device subscribed is reached by the token path, which is why `dtelco-message` falls back to a token send on code 11 |
