-- The reset clears the recommendation send list rows too.
--
-- Without this, a rehearsal opens with the previous room's three products already on the contact
-- and already queued for the Dengage send list. The recommendation is the one thing in this
-- demonstration whose whole claim is "these are the three the site just chose for you", so three
-- chosen yesterday are worse here than anywhere else: the rail would show one set and the send
-- list another, and the difference is exactly what a prospect is being asked to believe.
--
-- Cleared by the `simulated` marker, like dtelco_watch and dtelco_offline_event, rather than by a
-- date: the seeded dates roll forward and a date cut off clears the wrong side of them.
create or replace function public.dtelco_reset_demo()
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
declare p int; v int; u int; e int; t int; b int; w int; l int; m int; r int; cut timestamptz;
begin
  ...
  -- The recommendation send list rows. Nothing in Dengage is deleted by this: the rows already
  -- pushed into dtelco_reco_list stay, because nothing in this build deletes from the account.
  -- What clears is the queue this side, so the next sync sends what the next room actually saw.
  ...
  return jsonb_build_object('opening_state_taken_at', cut,
    ...
    'own_messages_cleared', m, 'recommendations_cleared', r);
end $$;
